/**
 * Export schedule for a specified year to scripts/exports/[timestamp]/[year].json
 * Usage:
 *   node scripts/export-schedule.mjs 2026
 */
...
async function run() {
  ...
  if (!yearArg || !/^\d{4}$/.test(yearArg)) {
    console.error("Usage: node scripts/export-schedule.mjs <year>");
    process.exit(1);
  }
  ...
  const scheduleJson = scheduleSnap.docs.map((docSnap) => {
    const data = docSnap.data();
    return {
      date: String(data.date ?? ""),
      talkId: Number.isInteger(data.talkId) ? data.talkId : null,
      customTalkTitle: String(data.customTalkTitle ?? ""),
      speakerId: typeof data.speakerId === "string" ? data.speakerId : null,
      status: String(data.status ?? "open"),
      notes: String(data.notes ?? ""),
    };
  });

  writeJson(outputDir, `${year}.json`, scheduleJson);
  console.log(`Export output directory: ${outputDir}`);
  console.log(`Exported schedule entries: ${scheduleJson.length}`);
}
